 ...
 */
@Repository
public class NotFriendsOfGuestRepository implements FindAllOfEntityRepository<KorisnikRepo,String> {
    @Override
    public Iterable<KorisnikRepo> findAll(String email) {
        ...
        List<KorisnikEntity> list = session.createNativeQuery("select * from korisnik where korisnik.EMAIL not in\n" +
                "(\n" +
